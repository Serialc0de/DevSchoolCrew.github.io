	...
	frontend: {
		...
		toggleCreation: function () {
            ...
            (function directoryEdit() {
                if ($('#directory-edit').length === 0)
                    return;

                $("#directory-edit, #directory-loader").hide();

                $("#show-directory-edit").on('click', function () {
                    var loader = new Woffice.loader($('.frontend-wrapper'));
                    $("#show-directory-edit").hide();

                    function show_edit_directory() {
                        $("#directory-edit").show();
                        loader.remove();
                    }

                    setTimeout(show_edit_directory, 1000);
                });

                $("#hide-directory-edit").on('click', function () {
                    var loader = new Woffice.loader($('.frontend-wrapper'));
                    $("#directory-edit").hide();

                    function hide_edit_directory() {
                        $("#show-directory-edit").show();
                        loader.remove();
                    }

                    setTimeout(hide_edit_directory, 1000);
                });
            }());
