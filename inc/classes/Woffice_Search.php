...
if( ! class_exists( 'Woffice_Search' ) ) {
	class Woffice_Search
	{
		...
		{
			$this->search = strtolower(esc_attr__($_POST['search']));
			...
			foreach ($this->types as $type) {
				...
				// WordPress case
					...
					while($loop->have_posts()) {
						...
						$meta = get_the_date() .' &mdash; '. __('by') .' '. get_the_author();
		...
		private function format($item, $type)
		{
			...
			if ($type === 'members') {
				$item['meta'] = __('Joined') . ' '. date('M Y', strtotime(get_userdata($item['id'])->user_registered));
			}
