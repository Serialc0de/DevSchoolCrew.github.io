...
if(!function_exists('woffice_members_filter')) {
    ...
    function woffice_members_filter()
    {
        ...
        if ($buddy_filter == "show") {
            ...
            echo '<button id="woffice-members-filter-btn" type="button" class="btn btn-default mt-0" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
            echo '<i class="fa fa-users"></i>';
            _e("Select Members", "woffice");
            echo '<i class="fa fa-caret-down"></i>';
            echo '</button>';
            echo '<ul class="dropdown-menu" role="menu">';
            echo '<li><a href="javascript:void(0)" data-role="0"  class="dropdown-item">' . esc_html__('All members', 'woffice') . '</a></li>';
            global $wp_roles;
            ...
            foreach ($wp_roles->roles as $key => $value):
                if (substr($key, 0, 4) != 'bbp_' && !in_array($key, $buddy_excluded_directory)
                    ...
                    echo '<li><a href="javascript:void(0)" data-role="' . esc_attr($key) . '" class="dropdown-item">' . esc_html($value['name']) . '</a></li>';
                }
            endforeach;
            echo '</ul>';
            echo '</form>';
...
if(!function_exists('woffice_list_xprofile_fields')) {
    ...
    function woffice_list_xprofile_fields($user_ID)
    {
	    ...
	    if ( ! empty( $formatted_social_items ) ) {
		    echo '<div class="member-xprofile-social-items text-center mt-2">';
